...
def process_variants(file_paths, samples):
    ...
    for file_path, sample in zip(file_paths, samples):
        temp = pd.read_csv(file_path, sep="\t", names=["chr", "start", "end", "name","type","length", "genotype"])
        temp["sample"] = sample
    ...
    variants_plot["length"] = variants_plot["length"].apply(lambda x: np.abs(int(x)))
...
def plot_stacked_prop(df, sample, ax):
    ...
    subset_pivot.sort_values("length_category", ascending=False).plot(kind='barh', stacked=True, ax=ax, width = 0.9, label=sample, color=["#00A074","#4A6AA2", "#D2520D"])
...
# variants_txt = '/home/mylequis/Documents/Doctorado/Papers/Genome_report/00_DATA/Figures_ideograms/processed/CBFH00195_sniffles_clean.txt'
# variants2_txt = '/home/mylequis/Documents/Doctorado/Papers/Genome_report/00_DATA/Figures_ideograms/processed/CBFH00251_sniffles_clean.txt' 
# file_paths = [ variants_txt, variants2_txt]
...
# ghost_axis(ghost1, "SV length distribution", 30)
...
################################ subset
################################ Figure SV distribution version 2
...
variants_plot.groupby(["sample","type"]).describe()["length"]["mean"]
variants_plot.groupby(["sample","type"]).describe()["start"]["count"]
variants_plot.groupby(["sample","type"])["length"].sum()

variants_plot = process_variants(file_paths, samples)

fig = plt.figure(figsize=(18, 20))
gs = GridSpec(5, 5, figure=fig, hspace=0.5)
...
ax1_2 = fig.add_subplot(gs[2:4, 0:4])
ax1_leg = fig.add_subplot(gs[0:3,4])
ax1_divider = make_axes_locatable(ax1_leg)
ax1_leg1 = ax1_divider.append_axes("top", size="100%", pad="300%")

# ax0 = fig.add_subplot(gs[1, 4])
ax2 = fig.add_subplot(gs[4, 0:2])
ax3 = fig.add_subplot(gs[4, 2:4])
labels1 = fig.add_subplot(gs[4,4])
ghost1 = fig.add_subplot(gs[4, 0:4])
...
ax1.imshow(heatmap1, aspect=0.45)
# ax1.set_xlim(right=2050, left=400)
# ax1.set_ylim(bottom=2005, top=600)
mod_hatches(ax1)
ax1.set_title("Genome-wide SV distribution", pad=30, size=20,fontweight='bold')
ax1.set_yticks([])
ax1.set_xticks(range(69, 1900,157), list(range(1,13)))

ax1_2.imshow(heatmap, aspect=0.45)
# ax1_2.set_xlim(right=2050, left=400)
# ax1_2.set_ylim(bottom=2005, top=600)
mod_hatches(ax1_2)
ax1_2.set_yticks([])
...
plot_scale(ax1_leg, ax1_leg1)
ax1_leg1.set_title("Normalized density", fontsize=14)
ax1_leg.set_title("Normalized density", fontsize=14)
...
########### legends and labels
# remove al innecesary lines
format_axes([ax1,ax1_2,ax2,ax3,labels1])

ax2.legend().remove()
ax2.set_title("$C. missioneira$", size=16)
ax2.yaxis.set_visible(False)
ax2.set_xlabel("Proportion of total SV count", fontsize="14")
ax3.set_title("$C. tuca$",size=16)
ax3.yaxis.tick_right()
ax3.set_ylabel("")
ax3.set_xlabel("Proportion of total SV count", fontsize="14")
...
labels1.xaxis.set_visible(False)
labels1.set_facecolor('none')
plt.savefig(path + "SV_genome_report.svg", dpi=600)
...
def plot_proportions(ax, title, dataset):
    ...
    # Calculate proportions
    proportions = [dataset[0]/dataset[2],
                   dataset[1]/dataset[3]]
...
def plot_expected_vs_observed(ax, title, featureprop, nonfeatureprop, bar_width=0.8):
    categories = ["Obs.", "Exp."]
    # Stacked bar plot
    ax.bar(categories, featureprop, label="Feature", color=["#D2520D","darkgray"], width=bar_width)
    ax.bar(categories, nonfeatureprop, bottom=featureprop, label="Non-Feature", color=["#D2520D","darkgray"], hatch="//", edgecolor='white', width=bar_width)
    ax.set_ylabel(title)
...
gs = GridSpec(3, 3, figure=fig, height_ratios=(1,0.5,0.5))
...
plot_proportions(ax1, "Intergenic | Genic", intergenic)
plot_proportions(ax2, "Intronic | Exonic", intronic)
plot_proportions(ax3, "Repeats | Non-repeats", repeats)

format_axes2([ax1,ax2,ax3])
...
for i in [ax1, ax2]:
    i.tick_params(left=False, labelleft=False, right=False, labelright=False)
...
counts["Non repetitive"] = 100 - counts["Non repetitive"]
counts.rename(columns={"Unknown":"Unclassified"}, inplace=True)
counts=counts.loc[:,order2]
barplot = counts.plot.barh(stacked=True, color=color_list_8, ax=ax4)
...
barplot.set_ylabel("Genome wide")
barplot.set_yticklabels([])
barplot.xaxis.set_visible(False)
legend = ax4.legend(loc='center', ncol=4, frameon=False)

SVrepeatsdf = pd.DataFrame.from_dict(Ctuca_results3['Repeat type lengths (SVs)'], orient="index")
Ctuca_results3["Repeat bases in SVs"]


SVrepeatsdf = SVrepeatsdf.rename(columns={0:"Proportion"})
SVrepeatsdf = SVrepeatsdf.T
SVrepeatsdf = SVrepeatsdf.rename(columns={"Unknown":"Unclassified","Satellite":"Satellites","LINE":"LINEs","simple_repeat|Simple_repeat":'Simple & Low compl',"RC|Rolling":'Rolling-circles'})
SVrepeatsdf["Non repetitive"] = Ctuca_results["Total SV bases"] - SVrepeatsdf.sum(axis=1)
SVrepeatsdf = SVrepeatsdf.loc[:,order2]
SVrepeatsdf = 100 * SVrepeatsdf / SVrepeatsdf.sum(axis=1)[0]
barplot2 = SVrepeatsdf.plot.barh(stacked=True, color=color_list_8, ax=ax5)
barplot2.spines['top'].set_visible(False)
barplot2.spines['right'].set_visible(False)
barplot2.spines['left'].set_visible(False)
barplot2.set_xlabel("Genome proportion (%)")
barplot2.set_yticklabels([])
barplot2.legend().set_visible(False)
barplot2.set_ylabel("SV regions")
outputpath = '/home/mylequis/Documents/Doctorado/Papers/Genome_report/00_DATA/Figure5/'
plt.savefig(outputpath + "SV_repeats.svg", dpi=600)
